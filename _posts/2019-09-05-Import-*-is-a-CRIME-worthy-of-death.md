...
#### Namespaces
Python has a very pythonic take on namespacing and scope resolution.

{% include techquote.html quote="A namespace is a mapping from names to objects. Most namespaces are <u>currently implemented as Python dictionaries</u>..." footer="Python Documentation - 9.2 Scopes and Namespaces" %}

Moreover, given that the **keys of a dictionary form a set**, i.e. keys must be unique, **this ensures that all identifiers in a namespace are unique**. 

Examples of namespaces in Python would be the set of builtin names ```__builtins__```, containing all of Python's out-of-the-box functions such as ```sum()``` or ```list()```, the global names, such as global identifiers or imports at the top of a Python file, and local names which we find within a block, such as a for loop variable[^for_loop_scope] or the local variables of a function. So all the identifiers which are found after executing an ```import``` statement at the top of a Python file end up in the global namespace. Consider the example given at the beginning of this article:



The ```import *``` statement in ```helper.py``` has resulted in all the functions from ```pyspark.sql.functions``` being added to the global namespace. This is akin to the behaviour expected when ```using``` an entire namespace in a header file and then including that header file in a .cpp file, as I suggested earlier.
...
Python's scope resolution uses the LEGB rule, as described in <span class="inline-links">[Learning Python]()</span>: 
1.
2.
3.
4.

Typical scope resolution diagram

Hence why the ```pyspark``` sum function is called instead of the inbuilt one.

In Python, the scope resolution is also the reason why we cannot have private variables - we cannot control access to pairs within the global() dictionary so all objects which are created can be found within it by the Python scope resolution mechanism.

#### How to solve our problem?
So this problem seems to be a combination of 3 things:
1. Python's unique scoping mechanism.
2. The fact that Spark's Scala interface has been kept exactly the same in Python without much thought. 
3. The use of ```import *``` making the error much harder to debug. 

We could solve this by changing the line ```from pyspark.sql.function import *``` to ```from pyspark.sql.function import sum as __sum```. CHECK IF NAME MANGLING WILL HIDE THE __SUM FROM THE OUTER FUNCTION.
